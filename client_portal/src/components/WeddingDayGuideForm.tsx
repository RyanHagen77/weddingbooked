...
const WeddingDayGuideForm: React.FC<WeddingDayGuideFormProps> = ({ contractId }) => {
  ...
  useEffect(() => {
    if (contractId) {
      ...
      fetch(`http://127.0.0.1:8000/contracts/api/wedding_day_guide/${contractId}/`, {
        headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/contracts/api/wedding_day_guide/${contractId}/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
